...
export const RoomChecklist: React.FC = () => {
  ...
  const handleAddTask = () => {
    ...
    const updated = {
      ...housekeepingForRoom,
      tasks: [...housekeepingForRoom.tasks, newTask],
    };
  ...
  const handleDeleteTask = (taskId: string) => {
    ...
    const updated = {
      ...housekeepingForRoom,
      tasks: housekeepingForRoom.tasks.filter((t) => t.id !== taskId),
    };
    dispatch({ type: "UPDATE_HOUSEKEEPING", payload: updated });
  };
      ...
      <Card title="Room Checklist">
        {!housekeepingForRoom ? (
          <div className="text-sm text-slate-500 dark:text-gray-400">
            No checklist found for this room.
          </div>
        ) : (
            ...
            <div className="space-y-2">
              {housekeepingForRoom.tasks.length === 0 ? (
                ...
                housekeepingForRoom.tasks.map((task) => (
                  <div
                    key={task.id}
                    className="flex items-center justify-between p-2 rounded-md border border-dark-700"
                  >
