...
const hotelReducer = (state: HotelState, action: HotelAction): HotelState => {
  switch (action.type) {
    ...
    case "UPDATE_EVENT_PACKAGE":
      return {
        ...state,
        eventPackages: state.eventPackages.map((ep) =>
    ...
    case "DELETE_HALL":
      return {
        ...state,
        halls: state.halls.filter((hall) => hall.id !== action.payload),
      };
    default:
      return state;
  }
...
export const HotelProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const initializeData = () => {
    ...
    if (isFirstLoad || customers.length === 0) {
      ...
      setStorageItem(storageKeys.events, defaultState.events);
      setStorageItem(storageKeys.eventPackages, defaultState.eventPackages);
    }
  };
